...
function loyverse_delete_objects(){
        ...
        <pre> Starting integrity check! </pre>
    ...
    ?>
        ...
        <pre> Integrity check done! Don't forget to uncheck the integrity check box in the settings!</pre>               

    <?php 
    $this ->write_to_loyverse_sync_log('Integrity check done! Don\'t forget to uncheck the integrity check box in the settings!');
    exit;

 }

function get_category_from_woocommerce($catid){
    global $wpdb;
    $tablename = get_option('lvs_table','1');
    $thedatabase = 'wp_'. str_replace('-', '_', $tablename);

    $sql = "SELECT wc_id FROM ". $thedatabase . " WHERE lv_id ='" . $catid ."'" ;
    $queryresults = $wpdb->get_results($sql);
    
    $wccatid = $queryresults[0] ->wc_id;
    return $wccatid;
...
function loyverse_sync(){ ?>  
    ...
    ?>   
        <pre> Starting Synchronization of categories and items.</pre>     
   
    <?php    

    $this ->write_to_loyverse_sync_log('Starting Synchronization of categories and items.');

    $cursor = 'null';
    $i=0;

 // Start Sync of Categories

  do{
    //$this ->write_to_loyverse_sync_log($i);  
    //$this ->write_to_loyverse_sync_log(get_option('lvs_catsync'));
    ...
    if(in_array('cursor',$loyverse_categories)){
        $cursor = $loyverse_categories[0]['cursor'];
        $this ->write_to_loyverse_sync_log($cursor);
        
    }else{
        $this ->write_to_loyverse_sync_log('No cursor.');
    ...
    $sql = "SELECT * FROM ". $thedatabase ." WHERE lv_desc = 'Category'";
    $queryresults = $wpdb->get_results($sql);
    ...
    foreach($loyverse_categories[0] as $loyverse_category){
        ...
        if (is_array($loyverse_category) || is_object($loyverse_category)) {
        foreach($loyverse_category as $category){
            
            if(count($loyverse_category) == $i){

                $this ->write_to_loyverse_sync_log('Get out of loop');
                break;
            }
            ...
            /**   Create stuff for woocommerce category */
                ...
                ?>
                    ...
                    <pre> Unable to save category <?php echo $category['name'] ?> as it exists in Woocommerce. Please delte it from Woocommerce. </pre>

                <?php 
                $this ->write_to_loyverse_sync_log('Unable to save category '.$category['name'].' as it exists in Woocommerce. Please delte it from Woocommerce. ');
 ...
 /* Sync Products from Loyverses */
    ...
    do{
        ...
        <?php   
        $this ->write_to_loyverse_sync_log('Got all Items from Loyverse...  ');
        ...
        foreach ($loyverse_items[0] as $loyverse_item) {
                ...
                foreach($loyverse_item as $item){
                    ...
                    $loyverse_catid = $item['category_id'];
                    $loyverse_catname = $this->get_loyverse_category_by_id($loyverse_catid); 
                    ...
                    $sql = "SELECT * FROM ". $thedatabase . " WHERE lv_id ='" . $item['category_id'] ."'" ;
                    $queryresults = $wpdb->get_results($sql);
                    
                    //print_r(count($queryresults));
                    if (strlen($item['category_id']) < 1){
                        ?>        
                            <pre> No category selected in Loyverse. Carrying on.</pre>
                        <?php 
                        $this ->write_to_loyverse_sync_log('No category selected in Loyverse. Carrying on.');
                    ...
                    foreach($queryresults as $qres){ 
                        ...
                        if($qres->lv_id===$loyverse_item_id){ 
                            ...
                            $prod_data = [
                                ...
                                'categories' =>[
                                    [
                                    'id' => (integer)$this->get_category_from_woocommerce($loyverse_catid)
